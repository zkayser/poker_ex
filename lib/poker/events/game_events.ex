defmodule PokerEx.GameEvents do
  ...
  def game_started(game_id, room) do
    ...
    {Endpoint.broadcast!("games:" <> game_id, "game_started", map), map}
  end
  ...
  def state_updated(game_id, update) do
    ...
    Endpoint.broadcast!("games:" <> game_id, "update", map)
  end
  ...
  def winner_message(game_id, message) do
    Endpoint.broadcast!("games:" <> game_id, "winner_message", %{message: message})
  end

  def present_winning_hand(:none, _, _, _), do: :ok

  def present_winning_hand(game_id, winning_hand, player, type) do
    cards = Enum.map(winning_hand, fn card -> Map.from_struct(card) end)

    Endpoint.broadcast!("games:" <> game_id, "present_winning_hand", %{
      cards: cards,
      winner: player,
      type: type
    })
  ...
  defp game_map(room) do
    ...
    players =
      if room.active == [] do
        ...
        Enum.map(room.active, fn {name, _} ->
    ...
    seating = %{
      seating: Enum.map(room.seating, fn {name, pos} -> %{name: name, position: pos} end)
    }

    player_hands = %{
      player_hands:
        Enum.map(room.player_hands, fn {player, [card1, card2]} ->
          %{player: player, hand: [Map.from_struct(card1), Map.from_struct(card2)]}
        end)
    }

defmodule PokerEx.Room do
  require Logger
  alias PokerEx.{Room, Player, Events, RewardManager, Deck, Hand, Repo}
  alias PokerEx.Room.{Updater, BetTracker}
  ...
  def terminate(_reason, _state, %Room{type: :public, chip_roll: chip_roll})
  ...
  def handle_event(
        {:call, from},
        {:join, player, chip_amount},
        :idle,
        %Room{seating: seating} = room
      )
  ...
  def handle_event(
      ...
      when length(seating) <= @seating_capacity do
    case player_can_join?(player, seating) do
      true ->
        {:ok, _} = Player.subtract_chips(player, chip_amount)
  ...
  def handle_event({:call, from}, {:join, _player}, state, room),
    do: {:next_state, state, room, [{:reply, from, room}]}
  ...
  def handle_event(
        ...
        {:leave, player},
        _state,
        %Room{seating: seating, active: active} = room
      )
  ...
  def handle_event(
        ...
        {:leave, player},
        state,
        %Room{active: active, seating: seating} = room
      )
      when length(active) == 2 and length(seating) > 2 and
             state in [:pre_flop, :flop, :turn, :river] do
    ...
    unless player in active_players do
      ...
      {:next_state, :game_over, update,
       [{:reply, from, update}, {:next_event, :internal, :handle_fold}]}
    end
  end
  ...
  def handle_event({:call, from}, {:leave, player}, state, %Room{seating: seating} = room)
    ...
    {:next_state, :game_over, update,
     [
       {:reply, from, :skip_update_message},
       {:next_event, :internal, :handle_fold},
       {:next_event, :internal, :send_clear_ui}
     ]}
  end

  def handle_event({:call, from}, {:leave, player}, _state, %Room{seating: seating} = room)
      when length(seating) == 2 do
    update =
      room
      |> Updater.chip_roll(player, :leaving)
      |> Updater.clear_room()
    ...
    {:next_state, :idle, update,
     [{:reply, from, :skip_update_message}, {:next_event, :internal, :send_clear_ui}]}
  end
  ...
  def handle_event(
        ...
        {:call, player},
        state,
        %Room{called: called, active: active} = room
      )
  ...
  def handle_event(
        ...
        {:call, player},
        state,
        %Room{all_in: all_in, folded: folded, seating: seating} = room
      )
      when length(all_in) + length(folded) == length(seating) - 1 and
             state in @non_terminal_states do
    update =
      room
    ...
    {:next_state, :game_over, update,
     [{:reply, from, :skip_update_message}, {:next_event, :internal, :handle_all_in}]}
  end

  def handle_event({:call, from}, {:call, player}, state, room)
  ...
  def handle_event({:call, from}, {:call, player}, :river, room) do
    ...
    {:next_state, :game_over, update,
     [{:reply, from, :skip_update_message}, {:next_event, :internal, :reward_winner}]}
  end
  ...
  def handle_event(
        ...
        {:fold, player},
        state,
        %Room{active: active, all_in: all_in} = room
      )
    ...
    {:next_state, :game_over, update,
     [{:reply, from, :skip_update_message}, {:next_event, :internal, :handle_fold}]}
  end
  ...
  def handle_event(
        ...
        {:check, player},
        state,
        %Room{to_call: call_amount, round: round, called: called, active: active} = room
      )
      when length(called) < length(active) - 1 do
    update =
  ...
  def handle_event(
        ...
        {:check, player},
        state,
        %Room{to_call: call_amount, round: round, all_in: all_in, active: active} = room
      )
  ...
  def handle_event(:internal, :reward_winner, :game_over, %Room{winner: nil} = room) do
    ...
    Events.present_winning_hand(
      room.room_id,
      update.winning_hand.best_hand,
      update.winner,
      update.winning_hand.type_string
    )
  ...
  def handle_event(
        ...
        :handle_fold,
        :game_over,
        %Room{all_in: all_in, active: active} = room
      )
  ...
  def handle_event(
        ...
        :handle_fold,
        :game_over,
        %Room{all_in: all_in, table: table} = room
      )
  ...
  def handle_event(:internal, :handle_all_in, :game_over, %Room{table: table} = room)
      when length(table) < 5 do
    update =
      room
      |> Updater.table()
  ...
  def handle_event(:internal, :set_round, :between_rounds, %Room{seating: seating} = room)
      when length(seating) > 1 do
    update_one =
      room
      |> round_transition(:between_rounds)
  ...
  def handle_event(event_type, event_content, state, data) do
    Logger.warn(
      "Unknown event: #{inspect(event_type)} with content: #{inspect(event_content)} in state: #{
        inspect(state)
      }"
    )

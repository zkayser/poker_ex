defmodule PokerEx.Deck do
	...
	def deal(%Deck{cards: cards, dealt: dealt}, number) 
		when not is_nil(dealt) and is_list(dealt) and length(dealt) > 0 and length(dealt) < 52 - number do
		{current, remaining} = Enum.split(cards, number)
		{current, %Deck{cards: remaining, dealt: [current | dealt]}}
	end
	
	def deal(_, _), do: raise "number must be less than or equal to the remaining number of cards in the deck"
end
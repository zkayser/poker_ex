defmodule LobbyChannelTest do
  ...
  test "a `player_count_updated` message is broadcast whenever a player joins or leaves a public room",
       context do
    ...
    {:ok, _, _} =
      subscribe_and_join(socket, PokerExWeb.GamesChannel, "games:game_1", %{
        "type" => "public",
        "amount" => 500
      })
  ...
  test "the channel pushes the correctly paginated list of rooms when receiving a `get_page` message",
       context do

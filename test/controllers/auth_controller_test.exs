defmodule PokerExWeb.AuthControllerTest do
	...
	test "returns a status of 200 on oauth_handler for valid provider and non-existing user", context do
		received_json = %{"name" => "some name", "facebook_id" => "#{Base.encode16(:crypto.strong_rand_bytes(8))}"}
		conn = post(context.conn, auth_path(context.conn, :oauth_handler, received_json))
		assert json_response(conn, 200)
		assert String.contains?(conn.resp_body, "player")
	end
	...
	test "returns unauthorized when facebook_id and player name do not match", context do
		received_json = %{"name" => "some different name", "facebook_id" => context.player.facebook_id}
		conn = post(context.conn, auth_path(context.conn, :oauth_handler, received_json))
